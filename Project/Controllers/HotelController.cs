...
namespace Project.Controllers
{
    public class HotelController : Controller
    {
        ...
        public HotelController(IHomeService homeService, UserManager<User> userManager)
        {
        ...
        public async Task<ActionResult> Index(FindModel findModel)
        {
            ...
            Hotel hotel = await homeService.getHotelById(findModel.Id);

            List<Photo> hotelPhotos = new List<Photo>();
            hotelPhotos.AddRange(hotel.photos);
            hotelPhotos.Add(hotel.mainPhoto);
            hotel.reviews.ForEach(review => hotelPhotos.AddRange(review.photos));
            ...
            ViewBag.hotel = hotel;
            ViewBag.reviews = isFind(findModel) ? await Find(findModel) : hotel.reviews;
        ...
        {
            Hotel hotel = await homeService.getHotelById(findModel.Id);
            List<Review> reviews = hotel.reviews;
        ...
        public async Task<ActionResult> AddPhoto(List<IFormFile> photo, int hotelId)
        {
            if (photo.Count == 0)
            {
            ...
            Hotel hotel = await homeService.getHotelById(hotelId);
            await homeService.changeHotelPhoto(hotel, photos);
